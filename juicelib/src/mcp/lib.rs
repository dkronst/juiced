...
use crate::mcp::device;
// TODO: This is silly. fix it.
use crate::mcp::lib::{Channel as McpChannel, Reading as McpReading, Result as McpResult};

macro_rules! declare_mcp3xxx {
    ($name:ident, $doc:tt, $resolution:tt, $channels:tt) => {
        #[doc = $doc]
        pub struct $name {
            device: crate::mcp::device::Device,
        }

        impl $name {
            pub fn new(spi: rppal::spi::Spi) -> McpResult<Self> {
                Ok(Self {
                    device: device::Device::new(
                        spi,
                        device::Channels::new($channels)?,
                        device::Resolution::new($resolution)?,
                    )?,
                })
            }
            ...
            pub fn differential_read(
                &mut self,
                plus_channel: McpChannel,
                minus_channel: McpChannel,
            ) -> McpResult<McpReading> {
...
pub enum Error {
    ...
    UnsupportedChannel(McpChannel),
    UnsupportedDifferentialCombination(McpChannel, McpChannel),
}
